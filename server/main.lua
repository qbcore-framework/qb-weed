...
AddEventHandler('qb-weed:server:harvestPlant', function(house, amount, plantName, plantId)
    ...
    if weedBag ~= nil then
        if weedBag.amount >= sndAmount then
            if house ~= nil then
                local result = exports.ghmattimysql:executeSync('SELECT * FROM house_plants WHERE plantid=@plantid AND building=@building', {['@plantid'] = plantId, ['@building'] = house}) 
...
AddEventHandler('qb-weed:server:foodPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local plantStats = exports.ghmattimysql:executeSync('SELECT * FROM house_plants WHERE building=@building AND sort=@sort AND plantid=@plantid', {['@building'] = house, ['@sort'] = plantName,['@plantid'] = tostring(plantId)})
